...
class Quaternion():
    def __init__(self, w=1., x=0., y=0., z=0., axis_angle=None, euler=None):
        ...
        if axis_angle is None and euler is None:
            ...
            cp = np.cos(pitch * 0.5)
            sp = np.sin(pitch * 0.5)
            #Using the euler to quaternion conversion formula
            # Fixed frame
    ...
    def to_mat(self):
        # for reference of this formula check module 3, lesson 1, slide 7
        v = np.array([self.x, self.y, self.z]).reshape(3,1)
        return (self.w ** 2 - np.dot(v.T, v)) * np.eye(3) + \
               2 * np.dot(v, v.T) + 2 * self.w * skew_symmetric(v)

    def to_euler(self):
        """Return as xyz (roll pitch yaw) Euler angles. Quaternion to euler conversion"""

...
#     axis = 0 means along the column and axis = 1 means working along the row.
    ctr = xyz.mean(axis=1)
#     print(ctr[:,np.newaxis]) #prints out a mean of each of x,y and z values. convert a column vector to row vector
#     print(ctr[:])    #prints out a column vector 
    normalized = xyz - ctr[:, np.newaxis]
    M = np.dot(normalized, normalized.T)
#     print(np.shape(M)) #3*3 matrix
#     p = np.linalg.svd(M)[0] #this prints out a 3*3 matrix
    p = np.linalg.svd(M)[0][:, -1]
#     print(p) # a 1*3 matrix
    d = np.matmul(p, ctr)

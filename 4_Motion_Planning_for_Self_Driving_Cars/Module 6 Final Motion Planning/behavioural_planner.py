...
class BehaviouralPlanner:
    ...
    def transition_state(self, waypoints, ego_state, closed_loop_speed):
        ...
        if self._state == FOLLOW_LANE:
            ...
            if stop_sign_found:
                ...
                print("Vehicle transitioned to decelerate to stop state")
            # ------------------------------------------------------------------
        ...
        # state.
            ...
            if closed_loop_speed < STOP_THRESHOLD:
                ...
                self._stop_count = 0
            # ------------------------------------------------------------------
                print("Vehicle Transitioned to stop state")
            # pass
        ...
        # the stop sign and transition to the next state.
            ...
            if self._stop_count == STOP_COUNTS:
                ...
                if not stop_sign_found:
                    self._state = FOLLOW_LANE
                    print('Vehicle transitioning to follow lane state')
                # --------------------------------------------------------------
                # pass
            ...
            # Otherwise, continue counting.
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                print('Still waiting at the stop sign')
                self._stop_count += 1
                # --------------------------------------------------------------
    ...
    def get_goal_index(self, waypoints, ego_state, closest_len, closest_index):
        ...
        Set to be the earliest waypoint that has accumulated arc length (including closest_len)
        that is greater than or equal to self._lookahead.
        ...
        # We already got the closest index and the length of the closest waypoint from the ego vehicle using the method get_closest_index.
        # The closest_index (wp index) and closest_len (arc_length) give us the starting waypoint from which we have to calculate the next
        # waypoint such that the arc length which is the distance from the ego vehicle to the closest waypoint (closest_len) plus the subsequent
        # waypoints after should be equal to or greater than our lookahead distance. We need arc length to be greater than our lookahead
        # distance because the car could then have its local planner move towards that waypoint based on its state and current lookahead.
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        while wp_index < len(waypoints) - 1:
            arc_length += np.sqrt((waypoints[wp_index+1][1] - waypoints[wp_index][1])**2 + (waypoints[wp_index+1][0] - waypoints[wp_index][0])**2)
            if arc_length > self._lookahead:
                wp_index +=1
                break
            else:
    ...
    def check_for_stop_signs(self, waypoints, closest_index, goal_index):
        ...
        for i in range(closest_index, goal_index):
            ...
            # stopsign_fence has the form [x0, y0, x1, y1]
            intersect_flag = False
            for stopsign_fence in self._stopsign_fences:
                wp_1   = np.array(waypoints[i][0:2])
                wp_2   = np.array(waypoints[i+1][0:2])
                s_1    = np.array(stopsign_fence[0:2]) #x0, y0
                s_2    = np.array(stopsign_fence[2:4]) #x1, y1
...
def get_closest_index(waypoints, ego_state):
    ...
    for i in range(len(waypoints)):
        distance = np.sqrt((waypoints[i][1] - ego_state[1])**2 + (waypoints[i][0] - ego_state[0])**2)

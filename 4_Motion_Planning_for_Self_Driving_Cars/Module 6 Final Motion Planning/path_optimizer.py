...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        p0 = [0.0, 0.0, sf_0] #initial conditions for the optimizer
    ...
    # arc lengths, and spiral parameters a(a0), b(a1), c(a2), d(a3).
    # Recall that the equation of a cubic spiral is
    # kappa(s) = a + b*s + c*s^2 + d*s^3, this equation can be written as
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        # d = np.array(d)
        # s = np.array(s)
        thetas = np.multiply(a, s) + (1/2)*np.multiply(b, np.square(s)) + (1/3)*np.multiply(c, np.power(s, 3)) + (1/4)*np.multiply(d, np.power(s, 4))
    ...
    def sample_spiral(self, p):
        ...
        # to the spiral parameter space. Here p consists of p0,p1,p2,p3 and p4
        p = [0.0, p[0], p[1], 0.0, p[2]]    # recall p0 and p3 are set to 0
                                            # and p4 is the final arc length
        # refer to module 7, lesson 2, slide 12 (parameter remapping)
        a = p[0]
    ...
    def objective(self, p):
        ...
        """
        #  the numbers, 25, 30, are weighting matrices.
        # min 𝑓_𝑏𝑒(𝑎0,𝑎1,𝑎2,𝑎3,𝑠𝑓) + 𝛼(𝑥_s(𝑠𝑓) − 𝑥_𝑓) + 𝛽(𝑦_s(𝑠𝑓) − 𝑦𝑓) + 𝛾(𝜃_s(𝑠𝑓) − 𝜃_𝑓)

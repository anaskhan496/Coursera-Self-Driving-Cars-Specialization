    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                circle_locations[:, 0] = path[0][j] + np.array(self._circle_offsets)*np.cos(path[2][j]) 
                circle_locations[:, 1] = path[1][j] + np.array(self._circle_offsets)*np.sin(path[2][j])
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # The exact choice of objective function is up to you.
                # To encourage robust obstacle avoidance, we will also need to add a term that 
                # penalizes how close the planned path is to other paths in the path set 
                # that are in collision with a static obstacle.
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            score += self._weight/(np.sqrt((paths[i][0][-1] - paths[j][0][-1])**2 + (paths[i][1][-1] - paths[j][1][-1])**2))  
                            # --------------------------------------------------

...
class Controller2D(object):
    ...
    def update_desired_speed(self):
        ...
        return self._desired_speed, self.minidx
    ...
    def update_controls(self):
        ...
        if self._start_control_loop:
            ...
            """
            errors = v_desired - self.vars.v_previous
            k_1s = self._kps + self._kis + self._kds
            k_2s = -self._kps - (2.0 * self._kds)
            k_3s = self._kds
            ...
            # a = waypoints[0][1] - ty
            # b = tx - waypoints[0][0]
            # c = ty*waypoints[0][0] - waypoints[0][1]*tx
            # cross_error = (a*x + b*y +c)/np.sqrt(a*a+b*b)
            # cross_track_steer = np.arctan2((gain*cross_error),(kv+v))
            # heading_error = np.arctan2(-a,b) - yaw
            # steer_output = heading_error + cross_track_steer
